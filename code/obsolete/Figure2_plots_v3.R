...
runCastle <- function(target_gem, source_gem, source_identities, nFeatures=200) {
  library(scater)  
  library(xgboost) 
  library(igraph)  
...
for (i in 1:length(thresholds)) {
  ...
  for (cell in colnames(ss2_gem_norm)) {
    ...
    for (gene in rownames(ss2_gem_norm)) {
      if (ss2_gem_norm[gene, cell] > quantile(unlist(ss2_gem_norm[gene, ]), thresholds[i])) { # This is working
...
# remove bulk cells
scID_output <- scid_multiclass(target_gem = ss2_gem, markers = markers, estimate_weights_from_target = TRUE)
