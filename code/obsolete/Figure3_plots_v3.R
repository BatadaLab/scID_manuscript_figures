...
pheatmap(gem_avg_9k, border="white", color = colorspace::diverge_hsv(50), 
         ...
         filename = "~/Dropbox/nizar_katerina2017/scID/cellreports/manuscript/Figures/Figure3/reference_heatmap.pdf")
...
pdf("~/Dropbox/nizar_katerina2017/scID/cellreports/manuscript/Figures/Figure3/marker_based_identification_target.pdf", width = 1.2, height = 2)
...
# Figure 3A right
pdf("~/Dropbox/nizar_katerina2017/scID/cellreports/manuscript/Figures/Figure3/Target_tsne.pdf", width = 2, height = 2)
...
pheatmap(nuclei_avg, border="white",color = colorspace::diverge_hcl(50, h=c(180, 70), c=70, l=c(70, 90)), 
         ...
         show_rownames = F, scale = "row", legend = F, width = 2, height = 2, 
         filename = "~/Dropbox/nizar_katerina2017/scID/cellreports/manuscript/Figures/Figure3/Seurat_target_heatmap.pdf")
...
for (i in 1:length(combClust)) {
  cells <- intersect(WhichCells(cells.combined, idents = levels(combClust)[i]), colnames(E18_9k))
  if (length(cells) >= 1) {
    avg_exp <- rowMeans(E18_9k[toupper(positive_markers$gene), cells, drop = FALSE])
  } else {
    next
  }
  for (j in 1:length(celltypes)) {
    gem_avg_ds[j,i] <- mean(na.omit(avg_exp[toupper(positive_markers$gene[which(positive_markers$cluster == levels(celltypes)[j])])]))
  }
}
...
pheatmap(gem_avg_ds, border="white", color = colorspace::diverge_hsv(50), cluster_rows = F,
         ...
         filename = "~/Dropbox/nizar_katerina2017/scID/cellreports/manuscript/Figures/Figure3/CCA_heatmap.pdf")
...
mnn_labels_reference <- mnn_labels$reference
...
for (i in 1:length(ncol)) {
  ...
  for (j in 1:length(celltypes)) {
    gem_avg[j,i] <- mean(na.omit(avg_exp[markers$gene[which(markers$cluster == levels(celltypes)[j])]]))
  }
}
rownames(c) <- paste("GS", levels(celltypes), sep = "_")
...
pheatmap(gem_avg, border="white", color = colorspace::diverge_hsv(50), 
         cluster_rows = F, cluster_cols = F, show_colnames = F, fontsize = 5, 
         border_color = F, show_rownames = F, scale = "row", legend = F, width = 2, height = 2, 
         filename = "~/Dropbox/nizar_katerina2017/scID/cellreports/manuscript/Figures/Figure3/MNN_heatmap.pdf")
...
#new_markers <- scID_output$markers[which(scID_output$markers$avg_logFC > 0), ]
markers_filt <- positive_markers[which(positive_markers$gene %in% rownames(E18_nuclei_cpm)), ]
...
pheatmap(gem_avg, border="white",color = colorspace::diverge_hcl(50, h=c(180, 70), c=70, l=c(70, 90)), 
         cluster_rows = F, cluster_cols = F, show_colnames = F, fontsize = 5, border_color = F,
         show_rownames = F, scale = "row", legend = F, width = 2, height = 2, 
         filename = "~/Dropbox/nizar_katerina2017/scID/cellreports/manuscript/Figures/Figure3/scID_heatmap.pdf")
...
pheatmap(gem_avg, border="white",color = colorspace::diverge_hcl(50, h=c(180, 70), c=70, l=c(70, 90)), 
         cluster_rows = F, cluster_cols = F, show_colnames = F, fontsize = 5, 
         border_color = F, show_rownames = F, scale = "row", legend = F, width = 2, height = 2, 
         filename = "~/Dropbox/nizar_katerina2017/scID/cellreports/manuscript/Figures/Figure3/scmap_heatmap.pdf")
...
pheatmap(gem_avg, border="white", color = colorspace::diverge_hcl(50, h=c(180, 70), c=70, l=c(70, 90)), 
         cluster_rows = F, cluster_cols = F, show_colnames = F, fontsize = 5, 
         border_color = F, show_rownames = F, scale = "row", legend = F, width = 2, height = 2, 
         filename = "~/Dropbox/nizar_katerina2017/scID/cellreports/manuscript/Figures/Figure3/castle_heatmap.pdf")
...
pdf("~/Dropbox/nizar_katerina2017/scID/cellreports/manuscript/Figures/Figure3/ARI.pdf", width = 1.5, height = 1.5)
...
pdf("~/Dropbox/nizar_katerina2017/scID/cellreports/manuscript/Figures/Figure3/VI.pdf", width = 1.5, height = 1.5)
ggplot(df, aes(y=value, x=threshold, fill=method)) + 
  geom_bar(stat="identity", position = "dodge") + scale_fill_jco() +
  theme(legend.position="", text = element_text(size=3), plot.margin = unit(c(0,0,0,0), "cm"), 
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
...
df$numbers <- factor(df$numbers, levels = c(0,1,2,4,12))
pdf("~/Dropbox/nizar_katerina2017/scID/cellreports/manuscript/Figures/Figure3/Num_signatures_alignment.pdf", width = 1.5, height = 2)
...
pdf("~/Dropbox/nizar_katerina2017/scID/cellreports/manuscript/Figures/Figure3/Num_signatures_mapping.pdf", width = 2, height = 2)
ggplot(df, aes(y=counts, x=method, fill=numbers)) + 
  geom_bar(stat="identity") + scale_fill_grey(start = 1, end = 0) +
  theme(legend.position="", text = element_text(size=3), plot.margin = unit(c(0,0,0,0), "cm"), 
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
